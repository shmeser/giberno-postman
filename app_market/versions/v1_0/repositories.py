...
class InsuranceRepository(MasterRepository):
    ...
    def issue_insurance_for_appeal(appeal, manager):
        ...
        _INSURED_DESCRIPTION = '''
            По Полису застрахован риск возникновения ответственности Страхователя, осуществляющего деятельность в качестве самозанятого,  на территории страхования в отведенные для такой деятельности периоды (часы), по обязательствам, возникшим вследствие причинения вреда имуществу заказчика работ в по договору оказания услуг.
        '''
        _BENEFICIARY = ''

        current_day = localtime(
            appeal.shift_active_date.replace(hour=0, minute=0, second=0),
        ...
        _SPECIAL_CONDITIONS = '''
            Если несчастный случай Застрахованного лица обусловил наступление последовательности событий, указанных в разделе Страховые риски настоящего Полиса, признанных страховыми случаями, то размер страховой выплаты по каждому очередному страховому риску из этой последовательности уменьшается на сумму страховой выплаты, произведенной ранее Страховщиком в связи с данным несчастным случаем.
            \n
            Не являются страховыми случаями любые события (убытки) и/или заявленные требования, произошедшие и/или предъявленные до даты вступления настоящего Полиса в силу; страховые выплаты по таким событиям по настоящему Полису не осуществляются.
            \n
            Не являются страховыми случаями любые события (убытки) и/или заявленные требования, произошедшие и/или предъявленные до даты вступления настоящего Полиса в силу; страховые выплаты по таким событиям по настоящему Полису не осуществляются.
        '''
        ...
        _RISKS = [
            {
                'insurance_risk': '10.1. Страхование Общей Гражданской Ответственности Страхователя. \nИмущественные интересы Страхователя, связанные с риском наступления гражданской ответственности за причинение вреда имуществу заказчика работ.',
                'conditions': [
                    {
                        'text': '''
                            Страхование Общей Гражданской Ответственности Страхователя в результате случаев, непреднамеренно возникших по вине Страхователя, осуществляющего деятельность в качестве самозанятого на оговоренной в Полисе территории:\n
                            Бой алкогольной продукции стоимостью от 5000 рублей за один предмет (общий лимит ответственности не может превышать 20 000 (двадцати тысяч) рублей.
                        ''',
                        'amount': 20000
                    },
                    {
                        'text': '''
                            Уничтожение или Повреждение торгового/складского оборудования*, торговой/складской мебели*, находящихся в торговом зале или складском помещении.
                        ''',
                        'amount': 100000
                    }
                ],
            ...
            {
                'insurance_risk': '10.3. Инвалидность в результате несчастного случая (для застрахованных лиц, являющихся резидентами РФ)',
                'conditions': [
                    {
                        'text': '''
                        100 % страховой суммы – при установлении I группы инвалидности
                        \n
            ...
            {
                'insurance_risk': '10.4. Полная утрата трудоспособности в результате несчастного случая * (для застрахованных лиц, являющихся нерезидентами РФ)',
                'conditions': [
                    {
                        'text': '''
                        Страховая выплата производится в соответствии с «Таблицей выплат при полной утрате трудоспособности» 
        ...
        _RISKS_DESCRIPTION = '''
            * При наступлении случая, имеющего признаки страхового, страховой риск «Полная утрата трудоспособности в результате несчастного случая» входит в страховое покрытие только в случае если Застрахованное лицо является нерезидентом РФ и действует только в отношении таких лиц. Для Застрахованных лиц - резидентов РФ выплата страхового возмещения осуществляться только по риску «Инвалидность в результате несчастного случая».
            \n\n
            Полный перечень страховых рисков, с наступлением которых возникает обязанность АО СК «Альянс» произвести страховую выплату, указан исключительно в Полисе. Перечень является поименованным, закрытым и исчерпывающим.
        '''

        insurance, created = ShiftAppealInsurance.objects.get_or_create(
            appeal=appeal,
            defaults={
                'number': _NUMBER,
                'insurer': _INSURER,
