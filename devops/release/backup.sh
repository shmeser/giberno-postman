...
PROJ_DIR=/srv/giberno-server_release
...
$GDRIVE_DIR/gdrive upload  --config $GDRIVE_DIR --service-account /release/gdrive-account.json  --delete --parent ${dir_id} /tmp/${postgres_db_archive_name}.tar.bz2
$GDRIVE_DIR/gdrive upload  --config $GDRIVE_DIR --service-account /release/gdrive-account.json  --delete --parent ${dir_id} /tmp/${files_archive_name}.tar.bz2

# необходимо удалить с google-диска старые бекапы, чтобы не забивать место

rotationDate=$(date -d "-3 day" +"%Y-%m-%d")
echo $rotationDate
files=$($GDRIVE_DIR/gdrive list --no-header --config $GDRIVE_DIR --service-account /release/gdrive-account.json --query "'$dir_id' in parents and modifiedTime < '$rotationDate'" | awk '{ print $1 }')

for file in $files
do
$GDRIVE_DIR/gdrive --config $GDRIVE_DIR --service-account /release/gdrive-account.json delete $file
done
